...
//   — daily-planner (`schedules.task`, cron '30 * * * *' = каждый час в :30
//     Тбилиси; ран часа H:30 ставит дроп часа H — модель почасовая, слот часа
//     H открывается в H:59 дня T) планирует дропы, но лишь пока
//     settings.planner_enabled в Supabase точно равен 'true': при любом другом
//     значении run() молча выходит. Активация боевого cron одобрена
//     пользователем и включена 04.08.2026 (docs/wiki/Runbook.md →
//     «Планировщик»); выключается тем же флагом, а не переделкой конфига.
//   — heartbeat (cron '12 19 * * *' = 23:12 Тбилиси) не бронирует ничего
//     вовсе: он сверяет план дня с квитанциями drop_reports и будит админов,
//     если вечер прошёл молча (docs/wiki/Runbook.md → «Heartbeat»).
//
