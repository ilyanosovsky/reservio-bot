 ...
 * худший баг проекта, поэтому в 23:12 Тбилиси (после закрытия дропов вплоть
 * до слота 22:00; дроп 23:00 к этому моменту ещё открыт и не проверяется)
...
import { dropDayOf, dropWatchWindow } from './scheduler.js';
...
/**
 * План дня, записанный планировщиком: `{date, at, slots:[{profileId,time}]}`
 * (см. formatPlannerPlan). Это ФАКТ — список ранов, которые планировщик реально
 * поставил, — и именно с ним сторож сверяет квитанции. Планировщик почасовой
 * (ран в H:30 ставит дропы часа H), поэтому план КОПИТСЯ по ранам одного дня
 * (mergePlannerPlan); `at` — момент последнего дописавшего рана.
 *
 * Зачем отдельный ключ, если правила лежат в БД: расписание и скипы живут своей
 * жизнью ВЕСЬ день. Владелец снимает скип с даты T+7 в 21:00 (первая же кнопка
 * меню «⏭ Скип» — ровно эта дата) или заводит новый сценарий в мастере — и
 * сторож, восстанавливающий план из живых правил в 23:12, ждёт квитанции по
 ...
 * 22:00 (квитанция ожидается с 23:06:30) время до крона сторожа в 23:12.
 */
 ...
 * числе между почасовыми ранами планировщика и кроном сторожа (23:12).
 * Отметку ставит КАЖДЫЙ ран, поэтому она отражает состояние флага в последний
 * час; дропы, поставленные раньше при включённом флаге, видны в записанном
 * плане дня — и сторож (runHeartbeat) ставит план выше этого решения.
 *
 *   — отметка сегодняшняя, без префикса: последний ран планировщика был
 *     ВКЛЮЧЁННЫМ, раны дропов поставлены. Если владелец выключил флаг в 21:40,
 *     поставленный в 21:30 ран всё равно отработает в 21:59 и забронирует корт
 *     (Runbook прямо об этом предупреждает) — и его отчёт обязан доехать до
 *     человека, иначе останется бронь на 80 GEL, о которой никто не знает;
 *   — отметка сегодняшняя, с префиксом 'disabled@': последний ран отработал
 *     выключенным и дропов не ставил — ждать квитанции не с чего (иначе вечер
 *     включения флага дал бы бессмысленные алерты по часам, которые никто не
 *     ставил);
...
/**
 * Ожидаемые квитанции по ЗАПИСАННОМУ плану дня (settings.planner_last_plan)
 * — основной путь: сверяем не с тем, что стоит в расписании сейчас, а с тем,
 * что планировщик реально поставил своими почасовыми ранами.
 *
 * Часы, чей дроп к моменту `now` ещё не закрылся (правило на 23:00, а сторож
 * тикает в 23:12), отсекаются так же, как в expectedReceipts. `labelOf` даёт
 ...
 *
 * На вход идёт результат selectEligibleRules из daily-planner.ts — то есть
 * применимость дня недели и скипы считаются РОВНО ТЕМ ЖЕ кодом, что у
 * планировщика: две независимые копии этих правил рано или поздно разъедутся,
 * и сторож начнёт будить админов из-за собственной ошибки. Планировщик
 * почасовой, так что любой час правила, чей дроп к `now` закрылся, обязан был
 * отчитаться — ран, который его не поставил, и есть повод для тревоги.
 *
