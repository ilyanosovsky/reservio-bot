...
Фазы 1–2 (протокол, booking engine, мультипрофили, облачный дроп через
trigger.dev + Supabase, исходящие Telegram-отчёты) готовы и покрыты тестами.
Фаза 3 — входящий Telegram-**бот** (`src/bot/`, команды и кнопки, БД-профили и
расписания в Supabase) реализована. Планировщик автоброней (`daily-planner`)
написан, но **выключен по умолчанию** — боевой cron включится только в
фазе 4 и только после явного одобрения пользователя. Подробности — в
`Architecture`, `Bot` и `Runbook`.
...
- [Bot](Bot.md) — команды и кнопки Telegram-бота, авторизация, админ-флоу
- [Hosting](Hosting.md) — сравнение хостингов для бота (Railway/Northflank/Vercel)
- [Dev-Process](Dev-Process.md) — ветки, PR, CI, ревью, мердж
- [Runbook](Runbook.md) — как гонять скрипты, читать логи, отменять брони, сидить профили и запускать бота
...
- Авторизация бота — allowlist Telegram `chat_id → профиль`; незнакомому
  chat_id бот отвечает полной тишиной. Никаких паролей/OAuth (`Bot.md`).
